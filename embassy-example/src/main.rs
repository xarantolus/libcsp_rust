...
use core::ffi::c_void;
use libcsp::{CspArch, CspConfig, Packet, Port, Dispatcher, CspInterface, interface, InterfaceHandle};
...
// ── EXPORT SYMBOLS MANUALLY (Since macro was being finicky) ──────────────────
...
#[no_mangle] pub extern "C" fn csp_bin_sem_remove(sem: *mut *mut c_void) -> i32 {
    unsafe { ARCH.bin_sem_remove(*sem) }; 1
}
#[no_mangle] pub extern "C" fn csp_bin_sem_wait(sem: *mut *mut c_void, timeout: u32) -> i32 {
    if unsafe { ARCH.bin_sem_wait(*sem, timeout) } { 1 } else { 0 }
}
#[no_mangle] pub extern "C" fn csp_bin_sem_post(sem: *mut *mut c_void) -> i32 {
    if unsafe { ARCH.bin_sem_post(*sem) } { 1 } else { 0 }
}
#[no_mangle] pub extern "C" fn csp_bin_sem_post_isr(sem: *mut *mut c_void, _px: *mut i32) -> i32 {
    if unsafe { ARCH.bin_sem_post(*sem) } { 1 } else { 0 }
}
...
#[no_mangle] pub extern "C" fn csp_mutex_remove(mutex: *mut *mut c_void) -> i32 {
    unsafe { ARCH.mutex_remove(*mutex) }; 1
}
#[no_mangle] pub extern "C" fn csp_mutex_lock(mutex: *mut *mut c_void, timeout: u32) -> i32 {
    if unsafe { ARCH.mutex_lock(*mutex, timeout) } { 1 } else { 0 }
}
#[no_mangle] pub extern "C" fn csp_mutex_unlock(mutex: *mut *mut c_void) -> i32 {
    if unsafe { ARCH.mutex_unlock(*mutex) } { 1 } else { 0 }
}

#[no_mangle] pub extern "C" fn csp_queue_create(length: i32, item_size: usize) -> *mut c_void {
    ARCH.queue_create(length as usize, item_size)
}
#[no_mangle] pub extern "C" fn csp_queue_remove(queue: *mut c_void) { ARCH.queue_remove(queue) }
#[no_mangle] pub extern "C" fn csp_queue_enqueue(queue: *mut c_void, item: *const c_void, timeout: u32) -> i32 {
    if ARCH.queue_enqueue(queue, item, timeout) { 1 } else { 0 }
}
#[no_mangle] pub extern "C" fn csp_queue_enqueue_isr(queue: *mut c_void, item: *const c_void, _px: *mut i32) -> i32 {
    if ARCH.queue_enqueue(queue, item, 0) { 1 } else { 0 }
}
#[no_mangle] pub extern "C" fn csp_queue_dequeue(queue: *mut c_void, item: *mut c_void, timeout: u32) -> i32 {
    if ARCH.queue_dequeue(queue, item, timeout) { 1 } else { 0 }
}
#[no_mangle] pub extern "C" fn csp_queue_dequeue_isr(queue: *mut c_void, item: *mut c_void, _px: *mut i32) -> i32 {
    if ARCH.queue_dequeue(queue, item, 0) { 1 } else { 0 }
}
...
#[no_mangle] pub extern "C" fn csp_calloc(nmemb: usize, size: usize) -> *mut c_void { ARCH.malloc(nmemb * size) }
...
#[no_mangle] pub extern "C" fn csp_clock_get_time(_t: *mut c_void) {}
#[no_mangle] pub extern "C" fn csp_clock_set_time(_t: *mut c_void) {}
#[no_mangle] pub extern "C" fn csp_sys_tasklist_size() -> i32 { 0 }
#[no_mangle] pub extern "C" fn csp_sys_tasklist(_o: *mut core::ffi::c_char) {}
