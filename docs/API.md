...
- **132 corpus records** in `corpus/ctest.jsonl`, each one an exchange a real libcsp node
  performed under `ctest/`'s **146 checks**, replayed against the port. `just mutants`
  reports how many of them some deliberate breakage can actually move — 104 at the time of
