...
- **492 tests** across the crates, in 10 binaries, with `--all-features`.
...
- **136 corpus records** in `corpus/ctest.jsonl`, each one an exchange a real libcsp node
  performed under `ctest/`'s **150 checks**, replayed against the port. `just mutants`
  reports how many of them some deliberate breakage can actually move — 108 at the time of
