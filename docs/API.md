...
- **116 corpus records** in `corpus/ctest.jsonl`, each one an exchange a real libcsp node
  performed under `ctest/`'s **137 checks**, replayed against the port. `just mutants`
  reports how many of them some deliberate breakage can actually move — 84 at the time of
