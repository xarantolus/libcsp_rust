...
- **583 tests** across the crates, in 50 binaries, with `--all-features`.
...
- **154 corpus records** in `corpus/ctest.jsonl`, each one an exchange a real libcsp node
  performed under `ctest/`'s **167 checks**, replayed against the port. `just mutants`
...
- **110 differential tests** in `difftest/`, millions of random inputs per run, linking the
