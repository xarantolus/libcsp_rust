...
use libcsp::{CspConfig, Packet, Socket, Priority, socket_opts, conn_opts, CspNode};
...
fn test_empty_packet_send() {
    ...
    let conn = node.connect(Priority::Norm, 1, port, 1000, 0)
    ...
    conn.send(pkt, 100).expect("Should be able to send empty packet");
...
fn test_maximum_packet_size() {
    ...
    pkt.write(&max_data).expect("Should be able to write max data");
...
fn test_buffer_exhaustion() {
    ...
    assert!(allocated > 0, "Should be able to allocate at least some packets");

    // Try to allocate one more - should fail
    let result = Packet::get(100);
    assert!(result.is_none(), "Should fail to allocate when pool is exhausted");
...
fn test_accept_timeout() {
    ...
    assert!(result.is_none(), "Accept should timeout when no connections");
}
...
fn test_read_timeout() {
    ...
    assert!(result.is_none(), "Read should timeout when no data available");
}
...
fn test_any_port_binding() {
    ...
    sock.bind(libcsp::ANY_PORT).expect("Should be able to bind to ANY port");
...
fn test_rdp_connection_properly_negotiated() {
    ...
    let server_thread = thread::spawn(move || {
        ...
        if let Some(conn) = sock.accept(2000) {
            ...
            assert!(conn.is_rdp(), "Server accepted connection should have RDP enabled");

            // Verify we can read data over RDP connection
            if let Some(pkt) = conn.read(1000) {
                assert_eq!(pkt.data(), b"rdp-data", "Should receive correct data over RDP");
    ...
    let conn = node.connect(Priority::Norm, 1, port, 1000, conn_opts::RDP)
    ...
    conn.send(pkt, 1000).expect("Failed to send over RDP connection");
...
fn test_rdp_prohibited_connection() {
    ...
    let server_thread = thread::spawn(move || {
        ...
        if let Some(conn) = sock.accept(2000) {
            ...
            assert!(!conn.is_rdp(), "Connection should not have RDP when client prohibits it");

            if let Some(pkt) = conn.read(1000) {
                assert_eq!(pkt.data(), b"no-rdp", "Should receive data over non-RDP connection");
                assert!(!pkt.is_rdp(), "Packet should not have RDP flag when RDP is prohibited");
            } else {
    ...
    let conn = node.connect(Priority::Norm, 1, port, 1000, conn_opts::NORDP)
        .expect("Non-RDP connection should succeed");

    // Verify connection does NOT have RDP flag
    assert!(!conn.is_rdp(), "Client connection should not have RDP when NORDP is specified");

    let mut pkt = Packet::get(10).expect("Failed to get packet");
    pkt.write(b"no-rdp").expect("Failed to write packet");
    conn.send(pkt, 1000).expect("Failed to send over non-RDP connection");
...
fn test_all_priority_levels() {
        ...
        for _ in 0..4 {
            if let Some(conn) = sock.accept(1000) {
                if let Some(pkt) = conn.read(100) {
                    ...
                    assert!(
                        matches!(prio, Priority::Critical | Priority::High | Priority::Norm | Priority::Low),
                        "Invalid priority: {:?}",
                        prio
                    );
                }
    ...
    for prio in [Priority::Critical, Priority::High, Priority::Norm, Priority::Low] {
        let conn = node.connect(prio, 1, port, 1000, 0)
            .expect("Failed to connect");
...
fn test_concurrent_connections() {
    ...
    for _ in 0..3 {
        ...
        let handle = thread::spawn(move || {
            let conn = node_clone.connect(Priority::Norm, 1, port, 1000, 0)
                .expect("Failed to connect");
